...
def bollinger_bands(df):
    ...
    # σ区間の境界線
    df['-' + str(buy_sigma) + 'σ'] = df['SMA'] - buy_sigma * df['std']
    df['+' + str(sell_sigma) + 'σ'] = df['SMA'] + sell_sigma * df['std']

    print('-' + str(buy_sigma) + 'σ: ' + str(df.iloc[-1]['-' + str(buy_sigma) + 'σ']))
    print('+' + str(sell_sigma) + 'σ: ' + str(df.iloc[-1]['+' + str(sell_sigma) + 'σ']))

    # 最新の値段が±xσ区間を超えているか判定
    buy_flg = df.iloc[-1]['close'] < df.iloc[-1]['-' + str(buy_sigma) + 'σ']
    sell_flg = df.iloc[-1]['close'] > df.iloc[-1]['+' + str(sell_sigma) + 'σ']
    return create_result(buy_flg, sell_flg)
